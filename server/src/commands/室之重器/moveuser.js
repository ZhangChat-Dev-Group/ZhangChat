...
export async function run(core, server, socket, data) {
  ...
  if (userExists.length > 0) {
    ...
    if (!data.force) return server.reply({
      cmd: 'warn',
      text: '不能移动该用户，因为目标频道内已经有人使用了他昵称了',
    }, socket);
    else {
      server.replyWarn('很抱歉，因管理员的强制操作，您即将被强制断开连接。', userExists[0])
      userExists[0].channel = undefined
  ...
  server.broadcast({
    cmd: 'info',
    text: `${badClient.nick} 被 ${socket.nick} 从 ?${socket.channel} 移入 ?${data.channel} 强制模式状态：${data.force ? '开' : '关'} 静默模式状态：${data.quiet ? '开' : '关'}`,
  ...
  server.broadcast({
    cmd:'info',
    text:`${badClient.nick} 被 ${socket.nick} 移入 ?${data.channel} 强制模式状态：${data.force ? '开' : '关'}  静默模式状态：${data.quiet ? '开' : '关'}`
...
export const info = {
  name: 'moveuser',
  description: '将目标用户移动到指定的频道，可选择开启强制模式和静默模式，即如果目标频道有占用目标用户的昵称的人则时自动断开占用者的连接和不告知目标用户被移动',
  usage: `
    API: { cmd: 'moveuser', nick: '<target nick>', channel: '<new channel>', force: true || false, quiet: true || false }
    文本：以聊天形式发送 /moveuser 目标昵称 目标频道 （如果在目标频道后面加一个空格，再加上随机的字符串，则开启强制模式） （如果开启强制模式后，再再其末尾加上随机字符串，则开启静默模式）`,
  runByChat: true,
  dataRules: [
    {
      name:'nick',
